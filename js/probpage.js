...
var probpageIdToTag = {}, probpageTagToId = {};
...
function renderprobpageProblemsPage(d, fir){
	...
	if(fir){
		delete(probpageIdToTag); probpageIdToTag = {};
		delete(probpageTagToId); probpageTagToId = {};
	}
	var passedQuestions = {};
	...
	if(D[0].length){
		for(var i=0; i<D[0].length; i++) if(probpageChosenTags.indexOf(probpageTagToId[D[0][i][0]]) == -1){
...
function probpageLoad(reList){
	...
	if(reList == undefined){
		...
		if(currentLoginHandle != ""){
			setLoaderById(1, generateAuthorizeURL(settings.codeforcesApiUrl + '/user.status', {handle: currentLoginHandle}), function(d){
