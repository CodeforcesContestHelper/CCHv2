...
function openSubmission(c, u) {
	...
	setTimeout(function() {
		$(".submissionContainer").css("opacity", 1);
	}, 100);
	...
	function copyJudge(str) {
		...
		let ret = $(`<span class='copyData' style="float: right"><span info='copy'>${languageOption.general.copy}</span></span>`);
		ret.attr("data", str + (str.endsWith("\n") ? "" : "\n"));
	...
	function submissionAddList(id) {
		...
		$(".submissionDataDetail").append(`<div style="margin-left: 5px; margin-right: 5px; margin-bottom: 10px; font-size: 18px; font-weight: bold">${localize("input")}${copyJudge(S[0])}</div>`);
		if (S[0] != undefined)
		...
		$(".submissionDataDetail").append(`<div style="margin-left: 5px; margin-right: 5px; margin-bottom: 10px; font-size: 18px; font-weight: bold">${localize("output")}${copyJudge(S[1])}</div>`);
		if (S[1] != undefined)
		...
		$(".submissionDataDetail").append(`<div style="margin-left: 5px; margin-right: 5px; margin-bottom: 10px; font-size: 18px; font-weight: bold">${localize("answer")}${copyJudge(S[2])}</div>`);
		if (S[2] != undefined)
		...
		$(".submissionDataDetail").append(`<div style="margin-left: 5px; margin-right: 5px; margin-bottom: 10px; font-size: 18px; font-weight: bold">${localize("result")}</div>`);
		if (S[3] != undefined)
