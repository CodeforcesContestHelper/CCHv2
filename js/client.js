...
DefaultStyle.chart.backgroundColor = {
	color: "#ddd"
};
...
DefaultStyle.plotOptions.series = {
	dataLabels: {
		color: '#000',
		style: {
			fontSize: '13px'
				...
				if (singleAnnouncementLength != q.length && q.length != 0) {
					for (var dis = q.length - singleAnnouncementLength - 1; dis >= 0; dis--) {
						...
						if (settings.openNotification)
							new Notification(`Notification of CF #${ci}`, {
								body: t,
								icon: '../favicon.png'
							});
					}
	...
	var reloadIf = function(url, callbacks, ID) {
		...
		contestRatingChangesHook = $.ajax({
			...
			success: function(json) {
				...
				for (var i = 0; i < json.length; i++)
					if (json[i].handle.toLowerCase() == un.toLowerCase()) {
...
function getAllSingleContestantInfo(currSingleLastTimeUpdate, un, ci, success, error, S, E, loadStandings) {
	...
	loadInfo(generateAuthorizeURL(settings.codeforcesApiUrl + "/contest.standings", {
		contestId: ci,
		handles: un,
	...
	loadInfo(generateAuthorizeURL(settings.codeforcesApiUrl + "/user.info", {
		handles: un
	}), ["result", "0"], 2, false);
	loadInfo(generateAuthorizeURL(settings.codeforcesApiUrl + "/contest.status", {
		contestId: ci,
		handle: un
	}), ["result"], 3, false);
	if (success.length != 4)
	...
	setTimeout(function() {
		...
		if (loadStandings && contestStandingLoadTime.getTime() < (new Date()).getTime() - settings.standingsLoadingGap) {
			...
			loadInfo(generateAuthorizeURL(settings.codeforcesApiUrl + "/contest.hacks", {
				contestId: ci
			}), ["result"], 4, true);
			if ((inContest == 2 && settings.openRankPredict >= 1) || (inContest >= 1 && settings.openRankPredict == 2))
				setTimeout(function() {
					loadInfo(generateAuthorizeURL(settings.codeforcesApiUrl + "/contest.standings", {
						contestId: ci,
						showUnofficial: settings.openRankPredict == 2
					}), ["result"], 5, false);
				}, 500);
		}
	}, 3000);
...
function flushRankDisplayer() {
	...
	if (contestRanks[contestRankChosen] == 0)
		...
		$(".singleRankChangesDisplayer").html(`<i class='fas fa-caret-up red'></i><span>${contestRankLast[contestRankChosen] - contestRanks[contestRankChosen]}</span>`);
	else
		$(".singleRankChangesDisplayer").html(`<i class='fas fa-caret-down green'></i><span>${contestRanks[contestRankChosen] - contestRankLast[contestRankChosen]}</span>`);
...
function flushContestantProgressBarInner() {
	...
	for (var i = 0; i < contestSubmissionList.length; i++) {
		...
		r.attr("sinfo", JSON.stringify([(q.creationTimeSeconds * 1000 - contestStartTime.getTime()) / l, q.problem.index, q.verdict, q.memoryConsumedBytes, q.timeConsumedMillis]));
...
function singleContestantSyncOfficialSettings(un, ci, json, p) {
	...
	if (json.contest.phase == "CODING") {
		singleContestantTimeCountdown(++singleContestantTimeCountdownTimeCnt);
...
function singleContestantMainTrack(currSingleLastTimeUpdate, un, ci) {
	...
	function func() {
		if (currSingleLastTimeUpdate != singleLastTimeUpdate) return;
		getAllSingleContestantInfo(currSingleLastTimeUpdate, un, ci, [singleContestantSyncOfficialSettings, singleContestantSyncUnofficialSettings, singleContestantSyncUserInfo, singleContestantSyncProblemStatus, singleContestantSyncHacks, singleContestantSyncStandings], [function() {}, function() {}, function() {}, function() {}, function() {}, function() {}], function() {
			if (currSingleLastTimeUpdate != singleLastTimeUpdate) return;
...
function singleVirtualMainTrack(currSingleLastTimeUpdate, un, ci, tm) {
	...
	function func() {
		if (currSingleLastTimeUpdate != singleLastTimeUpdate) return;
		getAllSingleContestantInfo(currSingleLastTimeUpdate, un, ci, [function() {}, singleVirtualSyncUnofficialSettings, singleContestantSyncUserInfo, singleVirtualSyncProblemStatus], [function() {}, function() {}, function() {}, function() {}], function() {
			if (currSingleLastTimeUpdate != singleLastTimeUpdate) return;
			flushContestantProgressBarInner();
			$(".singleContent > div > div > .loadingInterface").css("opacity", 0);
			setTimeout(function() {
			...
			if (contestRunningStatus == "FINISHED") {
				if (!H)
					setTimeout(function() {
						loadStandingsService(un, ci, true);
					}, 1000);
			}
			if (contestRunningStatus == "CODING")
				setTimeout(func, settings.reloadTime);
...
function loadSingleContestantAll(un, ci) {
	...
	var reloadIf = function() {
		...
		reloadSingleMemoryUsed();
		$.ajax({
			url: generateAuthorizeURL(settings.codeforcesApiUrl + "/contest.standings", {
				contestId: ci,
				from: 1,
				count: 1,
				showUnofficial: true
...
$(".forceLoadStandings").click(function() {
	...
	contestStandingLoader = 0;
	contestStandingLoadTime = new Date();
	loadInfo(settings.codeforcesApiUrl + "/contest.hacks", {
		contestId: ci
	}, ["result"], 0, true);
	setTimeout(function() {
		loadInfo(settings.codeforcesApiUrl + "/contest.standings", {
			contestId: ci,
			showUnofficial: settings.openRankPredict == 2
		}, ["result"], 1, false);
	}, 500);
