...
function initProblemPageInfo(page, data, id) {
	...
	for (var i = 0; i < qq.length; i++) {
		if (qq.eq(i).attr("class") != "header" && qq.eq(i).attr("class") != "sample-tests") {
			...
			var inp = [],
				oup = [];
			qq.eq(i).find(".input pre").each(function() {
				inp.push($.trim($(this).html().replace(/<br>/g, '\n').replace(/<\/div>/g, '</div>\n')))
			});
			qq.eq(i).find(".output pre").each(function() {
				oup.push($.trim($(this).html().replace(/<br>/g, '\n').replace(/<\/div>/g, '</div>\n')))
			});
...
function loadProblem(x, info) {
	...
	var p = problemCurrentPageList.findIndex(function(q) {
		return q[0] == x
	});
...
var loadType = [
	function(x) {
		return `<span>${getProblemIndexes(x)[1]}</span>`
	},
	function() {
		return `<i class="fas ${loadTypes[1]}"></i>`;
	},
	function() {
		return `<i class="fas ${loadTypes[2]}"></i>`;
	},
	function() {
		return `<i class="fas ${loadTypes[3]}"></i>`;
	}
];
...
function sampleWrapper(x, y) {
	...
	for (var i = 0; i < x.length; i++)
		ret.push({
			"input": x[i] + (x[i].endsWith("\n") ? "" : "\n"),
			"output": y[i] + (y[i].endsWith("\n") ? "" : "\n")
		});
	return ret;
}
