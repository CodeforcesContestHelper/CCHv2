...
var contestListFilterResult = [],
	contestListSortResult = [];
...
function displayContestListPage() {
	...
	for (var i = l; i <= r; i++) {
		...
		var rtList = [`<span><span class="fas fa-clock"></span> ${localize('contestListStart')}</span>`, `<span class="red"><span class="fas fa-running"></span> ${localize('contestListRun')}</span>`, `<span class="green"><span class="fas fa-check"></span> ${localize('contestListEnd')}</span> | <span style="cursor: pointer; color: grey" onclick='contestStartVirtualIf(${x[1]})'><span class="fas fa-user-secret"></span> ${localize('contestListVirtual')}</span>`];
...
$(".contestSearch .searchPagesButton").eq(0).click(function() {
	var l = 1,
		r = Math.max(1, Math.ceil(contestListSortResult.length / contestPerPage));
	contestListCurrentPage = 1;
	displayContestListPage();
})
