...
var problemTagsToColor = {}, profileIdToTag = {}, profileTagToId = {};
...
function profileRefreshSubmissionListPages(){
	...
	r = Math.min(r, profileFilteredSubmission.length);
...
function profileDrawSubmissionFrame(q){
	...
	$(".profileSubmissionGraphItem").unbind("mouseover").mouseover(function(){
		...
		var L = 15 + 20 + 2 - 10, R = 15 + 20 + 2 + 16 * 52 + 10;
		var rL = 0;
	...
	function refreshSelectedBlocks(l, r, L, R){
		...
		function add(x){
			var l = Math.floor((x + DT) / 7), r = Math.max((x + DT) % 7);
